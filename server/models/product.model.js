...
const SizeSubSchema = new Schema({
  ...
  thicknessMeasurement: { type: String, enum: ["inch", "cm"], default: "inch" },
}, { _id: false });

const ProductSchema = new Schema({
  companyId: { type: Schema.Types.ObjectId, ref: "Company", required: true, index: true },
  sellerId: { type: Schema.Types.ObjectId, ref: "Seller", required: true, index: true }, // denormalized: speeds common reads
  ...
  category: { type: String, required: true, enum: ["Granite", "Marble"], index: true },
  subCategory: { type: String, required: true, index: true },
  ...
  priceUnit: { type: String, enum: ["sq.ft", "sq.m", "piece"], default: "sq.ft" },
  description: { type: String, required: true, trim: true, minlength: 50, maxlength: 1500 },
  status: { type: String, enum: ["active", "inactive", "pending", "violation"], default: "pending", index: true },
}, { timestamps: true });

// compound indexes tuned for common queries
ProductSchema.index({ companyId: 1, status: 1, createdAt: -1 }); // list active products for a company, latest first
ProductSchema.index({ sellerId: 1, status: 1, createdAt: -1 }); // list products for a seller
ProductSchema.index({ category: 1, pricePerUnit: 1 }); // useful for category-based filters / sorting

// text index for search (weighted)
ProductSchema.index({ productName: "text", description: "text" }, { weights: { productName: 10, description: 2 } });

// partial index example: keep index small for only active products
ProductSchema.index({ companyId: 1, pricePerUnit: 1 }, { partialFilterExpression: { status: "active" } });
