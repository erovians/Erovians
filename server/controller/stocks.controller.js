  ...
  try {
    ...
    sheet.eachRow((row, rowNumber) => {
      ...
      const item = {
        ...
        qty: val(row.getCell(7)) != null ? Number(String(val(row.getCell(7))).replace(/,/g, "")) : null,
    ...
    if (stocks.length === 0) {
      fs.unlink(filePath, () => {});
      return res.status(400).json({ message: "No valid stock rows found (lot & material required)" });
    }
    ...
    const existingSet = new Set(existing.map((x) => x.lot));
    const finalList = matchAttachedStocks.filter((s) => !existingSet.has(s.lot));
