  ...
  return (
    <div className="h-full flex flex-col bg-white border-r">
      ...
      <div className="p-4 border-b">
        {/* <h2 className="text-xl font-semibold mb-3">Natan Chai</h2>
        <p className="text-sm text-gray-500 mb-3">Local Time: 6:14 PM</p>
         */}
        {/* Search */}
        <div className="relative">
          <input
            ...
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            className="w-full px-3 py-2 pr-8 border border-gray-300 rounded focus:outline-none focus:border-blue-500 text-sm"
          />
          <svg className="w-6 h-6 absolute right-3 top-1/2 -translate-y-1/2 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="flex border-b">
        <button 
          className={`flex-1 px-4 py-2 text-sm font-medium ${activeTab === 'all' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-600'}`}
          onClick={() => setActiveTab('all')}
        >
          All
        </button>
        <button 
          className={`flex-1 px-4 py-2 text-sm font-medium ${activeTab === 'unread' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-600'}`}
          onClick={() => setActiveTab('unread')}
        >
          Unread
        </button>
        <button 
          className={`flex-1 px-4 py-2 text-sm font-medium ${activeTab === 'reply' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-600'}`}
          onClick={() => setActiveTab('reply')}
        >
          To reply
        </button>
        <button 
          className={`flex-1 px-4 py-2 text-sm font-medium ${activeTab === 'follow' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-600'}`}
