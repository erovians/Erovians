...
export default function CertificateDialog() {
  const companyId="68e0f1eaf432b4351423b5d7" //"68e0f1eaf432b4351423b5d"
  const [open, setOpen] = useState(false);
  const [selectedType, setSelectedType] = useState("");
  const [form, setForm] = useState({
    certificationName: "",
  ...
  // Fetch certificates when dialog opens or companyId changes
  useEffect(() => {
    ...
    const fetchCertificates = async () => {
      try {
        ...
        setCertificates(res.data); // Adjust this according to your API response shape
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Refresh certificates list after upload
      const res2 = await api.get(`/company/${companyId}/certificates`);
        ...
        <DialogContent className="sm:max-w-2xl w-full">
          ...
          {certificates.length > 0 && (
            <div className="mb-4">
              ...
              <ul className="max-h-48 overflow-y-auto border rounded p-2 bg-gray-50">
                {certificates.map((cert) => (
                  <li key={cert.id} className="flex justify-between items-center mb-1">
                    <span>{cert.certificationName} ({cert.type})</span>
                    <a
                      href={cert.fileUrl}
                      target="_blank"
              ...
              <FieldSet>
                <FieldGroup>
                  <Field>
                    ...
                    <FieldDescription>
                      Accepted: PDF, JPG, PNG. Max size: (implement on server-side).
                    </FieldDescription>
                  </Field>
                </FieldGroup>
              </FieldSet>
