...
const steps = [
  ...
  { id: 4, title: "Review & Submit", component: ReviewStep, schema: null },
];
...
export default function CompanyProfileForm() {
  ...
  const handleSubmit = async () => {
    // Validate all steps before final submit (optional)
    try {
      await stepOneSchema.parseAsync(formData);
      await stepTwoSchema.parseAsync(formData);
      await stepThreeSchema.parseAsync(formData);
      setErrors({});
      // final submission: replace this with API call
      console.log("Final Submission", formData);
      alert("Submitted ✅ (check console)");
    } catch (err) {
      const formatted = mapZodErrors(err);
      setErrors(formatted);
      // jump to first failing step (nice UX)
      if (
        ...
        setCurrentStep(2);
      } else if (formatted["certificates"]) {
        setCurrentStep(3);
      }
    }
  };
